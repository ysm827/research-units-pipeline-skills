...
| JSON Schema-style compatibility | Version machine-readable artifacts before external consumers rely on them | `skill-audit-report.v1`, `doctor-report.v1`, `run-audit.v1`, `run-audit-diff.v1`, `harness-showcase-audit.v1`, `improvement-report.v1`, `validate_skill_audit_payload`, `validate_doctor_payload`, `validate_run_audit_payload`, `validate_run_audit_diff_payload`, `validate_showcase_audit_payload`, `validate_improvement_payload`, `docs/SKILL_AUDIT_SCHEMA.md`, `docs/DOCTOR_REPORT_SCHEMA.md`, `docs/RUN_AUDIT_SCHEMA.md`, `docs/RUN_AUDIT_DIFF_SCHEMA.md`, `docs/SHOWCASE_AUDIT_SCHEMA.md`, `docs/IMPROVEMENT_REPORT_SCHEMA.md` | `Partial` | Add formal JSON Schema files only if a non-Python consumer appears |
